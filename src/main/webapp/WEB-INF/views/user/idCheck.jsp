...
function setParentText() {
	var user_id = $("#user_id").val();
  opener.document.getElementById("input1-id").value = document.getElementById("user_id").value
...
<h2>중복체크</h2>
	<form action="${root }/user/searchIdCheck" id="searchIdForm" method="post">
		<input type="text" class="form-control" id="user_id" name="user_id" value="${user_id }" style="width:455px"/> 
		<button type="submit" class="btn_class" id="searchId">검색</button>
	</form>
	<div>
		<c:if test="${checkResultY != null}">
			${checkResultY} <br><br>
